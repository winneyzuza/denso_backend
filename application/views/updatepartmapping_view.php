    ...
    <script>
        $(document).ready(function(){
            $('#breakup').on('click', function(){
                $.ajax({
                    url:        '<?php echo base_url(); ?>index.php/update/partbreakup',
            ...
            $('#unite').on('click', function(){
                $.ajax({
                    url:        '<?php echo base_url(); ?>index.php/update/partunite',
                ...
                <select id="taken" size="10" multiple style="width:100%;">
                    <?php
                        foreach ($taken as $key => $value) {
                            echo "<option value='".$value['part_id']."'>".$value['name_eng']." (".$value['name_th'].")</option>";
                        }
                    ?>
                </select>
            </td>
